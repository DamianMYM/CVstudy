...
def count_tensor_elements(x: Tensor) -> int:
    ...
    if len(x.shape)==0:
        ...
        for i in range(x.shape[0]):
            num_elements += count_tensor_elements(x[i])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Compute the number of multiples of ten in the range
    num_multiples = ((stop - 1) // 10) - (start // 10) + int(start % 10 == 0)
    
    # Create an empty tensor if there are no multiples of ten
    if num_multiples == 0:
        return torch.empty((0,), dtype=torch.float64)
    
    # Otherwise, create a tensor of the multiples of ten
    else:
        # Compute the first multiple of ten in the range
        first_multiple = 10 * ((start + 9) // 10)
        
        # Create the tensor of multiples of ten
    ...
    # Otherwise, create a tensor of the multiples of ten
        ...
        # Create the tensor of multiples of ten
        x = torch.linspace(first_multiple, stop, num_multiples, dtype=torch.float64)
      '''  
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    #                      TODO: Implement this function                     #
    ##########################################################################
    # Get the last row of x
    last_row = x[-1]
    # Get the third column of x
    third_col = x[:, 2:3]
    # Get the data in the first two rows and first three columns of x
    first_two_rows_three_cols = x[:2, :3]
    # Get the elements in the even-valued rows and odd-valued columns of x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    #                      TODO: Implement this function                     #
    ##########################################################################
    # Set the first two rows and first three columns to [0, 1, 2, 2, 2, 2]
    x[:2, :3] = torch.tensor([[0, 1, 2], [0, 1, 2]], dtype=torch.int64)
    
    # Set the next two rows and first three columns to [3, 4, 3, 4, 5, 5]
    x[2:4, :3] = torch.tensor([[3, 4, 3], [3, 4, 5]], dtype=torch.int64)
    
    # Copy the first six columns to the next two columns
    x[:, 3:5] = x[:, :3]
    x[:, 5:7] = x[:, :2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Compute the minimum value of each row of x
    min_x, _ = torch.min(x, dim=1, keepdim=True)

    # Create a boolean mask where True corresponds to the minimum value of each
    # row of x
    mask = x == min_x

    # Set the minimum value of each row to 0
    y = x * (~mask)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    col_means = x.sum(dim=0) / x.shape[0]
    col_stddevs = ((x - col_means) ** 2).sum(dim=0) / x.shape[0]
    col_stddevs = (col_stddevs + 1e-8).sqrt()

    # Normalize each column by subtracting its mean and dividing by its standard deviation
    y = (x - col_means) / col_stddevs
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Calculate cumulative sums of the input tensors
    cum_sums = torch.cumsum(torch.cat([torch.zeros(1, dtype=x.dtype, device=x.device), xs[0]] + xs[1:]), dim=0)

    # Calculate means of the input tensors using the cumulative sums and input lengths
    y = (cum_sums[ls] - cum_sums[torch.cat([torch.zeros(1, dtype=ls.dtype, device=ls.device), ls[1:] - 1])]) / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Sort the input tensor and create a mask indicating which elements are unique
    ...
    # Calculate indices of first occurrences of unique values
    indices = torch.zeros_like(uniques)
    indices[1:] = torch.cumsum(unique_mask[:-1], dim=0)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
